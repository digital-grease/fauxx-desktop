...
# `native-host` SUBCOMMAND of the `fauxx` binary, so this thin wrapper invokes
...
# Absolute path to the installed `fauxx` binary (the apps/cli build).
FAUXX_BIN="/REPLACE/WITH/ABSOLUTE/PATH/TO/fauxx"
...
#
# These map to the `fauxx` global store flags (`--db`, `--passphrase-file`).
