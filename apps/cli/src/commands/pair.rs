...
pub async fn show(config: Config) -> anyhow::Result<()> {
    ...
    // The literal addresses, for the case where the phone cannot resolve our
    // mDNS `.local.` name and reports no route to a device on the same Wi-Fi
    // (issue #38). These are what a user types into the phone's
    // connect-by-address field; best candidate first.
    let endpoints = core.local_endpoints().await?;
    if endpoints.is_empty() {
        println!("addresses: (none found; this device may not be on a network)");
    } else {
        println!("addresses: {}", endpoints.join(", "));
    }
